...
// Integer pixel sizes for crystal clear rendering
const sizePixels: Record<AvatarOptimizedProps['size'] & string, number> = {
  xs: 24,
  sm: 32,
  md: 40,
  lg: 48,
...
// Text size classes
const textSizeClasses: Record<AvatarOptimizedProps['size'] & string, string> = {
  xs: 'text-xs',
  sm: 'text-sm',
  md: 'text-sm',
  lg: 'text-base',
...
const AvatarOptimized: React.FC<AvatarOptimizedProps> = React.memo(({
  userId,
  src,
  initials,
  size = 'md',
  ...
  // Check if URL is from Wasabi/supports Imgix
  const isWasabiUrl = finalSrc && (
    finalSrc.includes('wasabisys.com') || 
    finalSrc.includes('wasabi-proxy')
  );
      ...
      {finalSrc && (
        isWasabiUrl ? (
          <Imgix
            ...
            htmlAttributes={{
              alt: 'User avatar',
              className: 'object-cover img-locked w-full h-full rounded-full',
              onError: async () => {
...
AvatarOptimized.displayName = 'AvatarOptimized';
