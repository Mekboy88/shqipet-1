-- Normalize, dedupe, enforce uniqueness, and add atomic tab counter

-- 0) Normalize device_stable_id to lowercase for consistent keys
UPDATE public.user_sessions
SET device_stable_id = lower(device_stable_id)
WHERE device_stable_id <> lower(device_stable_id);

-- 1) Remove legacy duplicates to prepare for UNIQUE
WITH ranked AS (
  SELECT ctid,
         ROW_NUMBER() OVER (
...
-- 2) Ensure UNIQUE(user_id, device_stable_id)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 3) Ensure minimum tab count CHECK (idempotent)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    ...
    ALTER TABLE public.user_sessions
      ADD CONSTRAINT active_tabs_count_min_one CHECK (active_tabs_count >= 1);
  END IF;
END $$;

-- 4) Create atomic bump function used by the backend function
...
BEGIN
  ...
  VALUES (
    ...
    true, false, GREATEST(1, 1 + GREATEST(p_delta, 0)),
    v_city, v_country, v_country_code, v_region, v_lat, v_lon
  )
  ON CONFLICT (user_id, device_stable_id)
  DO UPDATE SET
    active_tabs_count = GREATEST(public.user_sessions.active_tabs_count + p_delta, 1),
